    ...
    public class Worklnfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns)) {
                string sql = "select workinfo.*,activity.activity.activityName from workinfo.activityId where workId=@id";
        ...
        public int GetFindCount(string workName) {
            ...
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where workverify='审核通过' and workName like concat('%',@workName like concat('%',@workName,'%')";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify,"+"userName=@userName,recommend=@recommend,recommentTime=@recommendTime where workId=@workId";
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workPictute=@workPicture where workId=@workId";
